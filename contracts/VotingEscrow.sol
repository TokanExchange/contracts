...
import {EPOCH_DURATION} from "contracts/Epoch.sol";

/// @title Voting Escrow
/// @notice veNFT Transfer functionality is essentially disabled. Only transfer to/from approved contracts/addresses is possible.
...
contract VotingEscrow is IVotingEscrow, IVotes {
    ...
    /// @dev addresses allowed to send/receive veTKN NFTs
    EnumerableSet.AddressSet private _transferWhitelist;

    /// @param _token_addr `TOKAN` token address
    /// @param _art_proxy art proxy address for nft
    constructor(address _token_addr, address _art_proxy) {
        initFlag = false;
        team = msg.sender;
        artProxy = _art_proxy;
        token = _token_addr;
    ...
    /// @param _ve_api veNft api contract address. Allows ve contract to check for claimable rewards before merging/spliting
    /// @param _team_msig set the team to msig
    function init(
        address _voter,
        address _ve_api,
        address _team_msig
    ...
    string public constant name = "veTokan";
    string public constant symbol = "veTKN";
    ...
    function _transferFrom(
        ...
        require(
            _from == address(0) ||
                _transferWhitelist.contains(_from) ||
                _transferWhitelist.contains(_to),
    ...
    event SetTransfersEnabled(bool _isEnabled);
    ...
    /**
     * @dev returns if "account" is allowed to send/receive veTKN
     */
    ...
    /**
     * @dev Set bool to allow tranfers
     */
    function updateTransfersEnabled(bool _isEnabled) external {
        require(msg.sender == team, "not team");

        isTransferEnabled = _isEnabled;
        emit SetTransfersEnabled(_isEnabled);
    }
}
